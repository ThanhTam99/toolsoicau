function formatNumber(number) {
    switch (number) {
        ...
        case 2:
            return `<span class="circle-purple">${number}</span>`;
        case 3:
            return `<span class="circle-red">${number}</span>`;
        case 4:
            return `<span class="circle-yellow">${number}</span>`;
        case 5:
            return `<span class="circle-blue">${number}</span>`;
        case 6:
            return `<span class="circle-pink">${number}</span>`;
        case 7:
            return `<span class="circle-lucbao">${number}</span>`;
        case 8:
    ...
    ketQuaMangCon.forEach((arr) => {
        ...
        for (let i = 0; i < maxRows; i++) {
            ...
            columns.forEach(subColumn => {
                if (subColumn[i] !== undefined) {
                    let cellContent = formatNumber(subColumn[i]);
...
document.getElementById('addInput').addEventListener('input', function (e) {
    ...
    this.value = input.replace(/[^1-8]/g, '');
});
...
document.getElementById('addForm').addEventListener('submit', function (event) {
    ...
    if (arrHandle.some(num => num < 1 || num > 8 || isNaN(num))) {
        alert('Vui lòng chỉ nhập số từ 1 đến 8.');
        return;
    }
    $.ajax({
